...
export interface ParseIntakeResult {
  ...
  paymentTerms?: string;
  /** Prazo de entrega (dias) lido do documento — repassado de volta em
   *  'submit' pra pré-preencher a data de entrega do pedido. */
...
export async function submitIntakeOrder(
  sessionToken: string,
  payload: { orderId: string; clientId: string; category: string; value: number; filePath: string; fileName: string; items?: ItemExtraido[]; paymentTerms?: string; deliveryLeadDays?: number }
): Promise<void> {
