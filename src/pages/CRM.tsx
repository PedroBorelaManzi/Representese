...
import { Search, MapPin, Building2, Trash2, ChevronRight, Plus, Loader2, FileUp, X, ChevronDown, Users, BellOff, Network, CheckSquare } from 'lucide-react';
...
export default function CRMPage() {
  ...
  const [view, setView] = useState<'clientes' | 'redes'>('clientes');
  ...
  return (
    <div className="h-[calc(100dvh-2rem)] flex flex-col">
      ...
      <div className="flex-1 bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 flex flex-col min-h-0 ring-1 ring-slate-200/80 shadow-none overflow-hidden relative">
        <div className="px-4 pt-3 flex items-center gap-2">
          <div className="flex bg-slate-100 dark:bg-zinc-800 rounded-full p-0.5">
            {([['clientes', 'Clientes', Users], ['redes', 'Redes', Network]] as const).map(([key, label, Icon]) => (
              <button
                key={key}
                onClick={() => { setView(key); if (key === 'redes') exitSelection(); }}
                className={cn("px-4 py-1.5 rounded-full text-xs font-black flex items-center gap-1.5 transition-all", view === key ? "bg-slate-900 text-white dark:bg-zinc-100 dark:text-zinc-900" : "text-slate-500")}
              >
          ...
          {view === 'clientes' && (
            <button
              ...
              className={cn("ml-auto px-3.5 py-1.5 rounded-full text-xs font-black flex items-center gap-1.5 border transition-all", selecting ? "bg-emerald-600 text-white border-emerald-600" : "text-slate-500 border-slate-200 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800")}
            >
              <CheckSquare className="w-3.5 h-3.5" /> {selecting ? 'Cancelar seleção' : 'Selecionar'}
            </button>
          )}
        </div>
        {view === 'clientes' && (
        ...
        {selecting && view === 'clientes' && (
          <div className="px-4 py-2.5 border-b border-emerald-200 dark:border-emerald-900/40 bg-emerald-50 dark:bg-emerald-500/10 flex items-center gap-3 text-xs font-black">
            <span className="text-emerald-700 dark:text-emerald-400">{selectedIds.size} selecionado{selectedIds.size === 1 ? '' : 's'}</span>
            <button onClick={() => setSelectedIds(new Set(filteredClients.map(c => c.id)))} className="text-slate-500 hover:text-emerald-700 underline">Selecionar todos ({filteredClients.length})</button>
            {selectedIds.size > 0 && <button onClick={() => setSelectedIds(new Set())} className="text-slate-500 hover:text-emerald-700 underline">Limpar</button>}
            <button
              onClick={() => setIsNetworkModalOpen(true)}
              disabled={selectedIds.size === 0}
              className="ml-auto px-4 py-1.5 bg-emerald-600 text-white rounded-full uppercase tracking-wide text-[10px] disabled:opacity-40 hover:bg-emerald-700"
            >
              Definir rede
            </button>
          </div>
              ...
              <div className="flex flex-col">
                 ...
                 {displayClients.map((client) => (
                    <Link
                      ...
                      onClick={selecting ? (e) => { e.preventDefault(); toggleSelected(client.id); } : undefined}
                      className={cn("p-4 cursor-pointer transition-colors hover:bg-slate-50/80 dark:hover:bg-zinc-800/40 border-b border-slate-100 dark:border-zinc-800/60 flex items-center gap-4 group", selecting && selectedIds.has(client.id) && "bg-emerald-50/70 dark:bg-emerald-500/10")}
                    >
                       {selecting && (
                          <input type="checkbox" checked={selectedIds.has(client.id)} readOnly tabIndex={-1} aria-label={`Selecionar ${client.name}`} className="w-4 h-4 accent-emerald-600 shrink-0 pointer-events-none" />
                       )}
                       ...
                       <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                             ...
                             {client.network_name && (
                               <span className="px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 text-[8px] font-black rounded-md uppercase whitespace-nowrap tracking-widest flex items-center gap-1" title="Rede">
                                 <Network className="w-2.5 h-2.5" />{client.network_name}
                               </span>
                             )}
      ...
      <NetworkAssignModal
        ...
        networks={existingNetworks(clients)}
        saving={savingNetwork}
        onClose={() => setIsNetworkModalOpen(false)}
        onConfirm={async (name) => {
          if (await assignNetwork(Array.from(selectedIds), name)) {
