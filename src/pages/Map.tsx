...
export default function Map() {
  ...
  /* ─── Planejar rota (até 10 paradas → abre no app de navegação) ──── */
        ...
        <AnimatePresence>
          {isRouteMode && routeClients.length > 0 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 40 }}
              className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000] bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl border border-slate-100 dark:border-zinc-800 p-5 w-[min(440px,calc(100%-32px))]"
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Rota · {routeClients.length} paradas</span>
