...
const RevenueChart = ({ data, loading, currentDate, onPrevMonth, onNextMonth }) => {
  ...
  const [savingCeiling, setSavingCeiling] = React.useState(false);
  ...
  const handleSaveCeiling = async () => {
    ...
    try {
      ...
      setCeilingInput('');
    } catch (err) { console.error(err); } finally { setSavingCeiling(false); }
  };
  ...
  const handleSaveGoals = async () => {
    ...
    try {
      ...
      setShowGoals(false);
    } catch (err) { console.error(err); } finally { setSavingGoals(false); }
  };
...
<div className="flex flex-col h-full relative" style={{ minHeight: '350px' }}>
        ...
        {showGoals && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute inset-x-0 top-0 z-[100] bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[32px] p-6 shadow-2xl max-h-[calc(100%-8px)] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowGoals(false)} className="p-2 hover:bg-slate-50 dark:hover:bg-zinc-800 rounded-full transition-colors shrink-0">
            ...
            <button onClick={handleSaveGoals} disabled={savingGoals} className="w-full mt-4 bg-slate-900 dark:bg-zinc-100 text-white dark:text-zinc-900 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        {showCeiling && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute inset-x-0 top-0 z-[100] bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[32px] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowCeiling(false)} className="p-2 hover:bg-slate-50 dark:hover:bg-zinc-800 rounded-full transition-colors">
                <X className="w-4 h-4 text-slate-400" />
              </button>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={handleSaveCeiling} className="bg-slate-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-6 rounded-2xl text-[10px] font-black uppercase disabled:opacity-50" disabled={savingCeiling}>{savingCeiling ? '...' : 'OK'}</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { setShowGoals(!showGoals); setShowCeiling(false); }} className="p-3 bg-slate-50 dark:bg-zinc-950 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-2xl transition-all" title="Configurar meta mensal por empresa">
            <Target className={cn("w-4 h-4 text-slate-400", showGoals && "text-emerald-600")} />
          </button>
          <button onClick={() => { setShowCeiling(!showCeiling); setShowGoals(false); }} className="p-3 bg-slate-50 dark:bg-zinc-950 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-2xl transition-all" title="Ajustar teto do gráfico">
            <Settings className={cn("w-4 h-4 text-slate-400", showCeiling && "rotate-45")} />
          </button>
        </div>
      </div>
